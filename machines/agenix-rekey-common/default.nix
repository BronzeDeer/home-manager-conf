{config, pkgs, ...}:
{
  ...
  # We use a yubikey as master identity, this needs extra support on the system
  services.pcscd.enable = true; #Allwos us to access the Yubikey via the SCard API (PC/SC)


  age.rekey = {
    # hostPubkey should be set by the individual machines
    #hostPubkey
    masterIdentities = [ ../../secrets/age-identities/age-yubikey-5c-primary-identity-c8d44732.pub ];
