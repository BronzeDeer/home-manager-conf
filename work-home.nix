...

{
  ...
  # TODO: this will add an extraneous compinit, because it doesn't detect the zplug one,
  # but we need this option to get completions for the rest of the system like sysctl etc
  # since we are using zsh-autocomplete, we can be certain that compinit is already tombstoned with a no-op anyway
